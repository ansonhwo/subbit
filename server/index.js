...
// plaidClient.getConnectUser(access_token, {}, (err, response) => {
  if (err) console.error(err)
    ...
    monthsByYear.pop()  // Data from last entry has nothing to compare against
    const filteredTransactions = transactionsByMonth.filter((transactionsForTheMonth, index) => {
      if (index === 0) {
        console.log(`transactions for the month of ${monthsByYear[index]}`)
        console.log(transactionsForTheMonth)
        console.log('\n\n')
      }
      if (index === transactionsByMonth.length - 1) return false
      else {
        return transactionsForTheMonth.filter(thisMonthsTransaction => {
          if (index === 0) {
            console.log(`transactions for the previous month of ${monthsByYear[index+1]}`)
            console.log(transactionsByMonth[index+1])
            console.log('\n\n')
          }
          let match = transactionsByMonth[index + 1].filter(lastMonthsTransaction => {
            let foundDate = moment(lastMonthsTransaction, 'YYYY-MM-DD').subtract(1, 'months').diff(thisMonthsTransaction.date, 'days')
            let dateLowerBound = foundDate - 5
            let dateUpperBound = foundDate + 5

            if (index === 0) {
              console.log('COMPARING')
              console.log(`${thisMonthsTransaction.name}, ${thisMonthsTransaction.date}`)
              console.log('\nAGAINST')
              console.log(`${lastMonthsTransaction.name}, ${lastMonthsTransaction.date}`)
              console.log(`This transaction is ${foundDate} days older than the current transaction.`)
              console.log('\n\n')
            }

            return (lastMonthsTransaction.name === thisMonthsTransaction.name
